...
user='user'
db='skill'
echo "create database ${db};" | mysql -uroot -p${passroot}
echo "DELETE FROM mysql.user WHERE User='root' AND Host NOT IN ('localhost', '127.0.0.1', '::1');" | mysql -uroot -phGS20dnlsFGfhsc3sf
